...
//
// 2026-09-10: 대기업을 다시 넣는다. 9월 9일에 "대기업은 기사가 나니 우리가 없어도 기록이 남는다" 며
// 뺐는데, 그 전제가 틀렸다. 기사는 개정 사실을 쓰지 조문을 보존하지 않고, 시민단체 자료는 그때그때
// 흩어진다. 어느 조가 어떻게 바뀌었는지를 시점별로 되짚을 수 있는 곳은 여전히 없다.
// 항목은 9월 7일·9일에 실측한 그대로 되살렸고 URL·셀렉터를 새로 재지 않았다 — 크론이 돌면 last_error 로
// 다시 잰다. 9월 10일에 넣은 중소 서비스 열둘은 그대로 둔다.
...
// 대기업 문서를 카탈로그로 되돌린 날. 항목은 2026-09-07·09 실측 그대로이고 URL·셀렉터를 새로 재지 않았다.
const RESTORED = '2026-09-10'
...
const ACTIVE: DocumentConfig[] = [
  ...
  // ── 중소 서비스 열둘 (2026-09-10). 감시가 덜하면서 소비자 분쟁이 실제로 터지는 업종에서 골랐다 —
