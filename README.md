...
- **변경 알림 메일** — 설정에서 켜면 관심 약관에 변경이 발행될 때 한 통씩 온다 (`src/notify.ts`). 발송은 Resend REST 호출 하나다: `npx wrangler secret put RESEND_API_KEY`, 보내는 주소는 `MAIL_FROM`(없으면 `CONTACT_EMAIL`; Resend 에 인증된 도메인이어야 한다). 키가 없으면 토글은 저장만 되고 아무것도 보내지 않는다. 본문에 발췌는 없고 시행·감지일, 중요도, 분류, 변경 페이지 링크뿐이다 (D-1). 알림 실패는 수집 실패가 아니라 로그로만 남는다.
